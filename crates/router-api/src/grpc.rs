...
fn proto_filter(filter: ProtoRouteFilter, principal: &Principal) -> Result<RouteFilter, ApiError> {
    ...
    Ok(RouteFilter {
        ...
        recipient_type: filter.recipient_type.map(Arc::from),
        recipient_identity: filter.recipient_identity.map(Arc::from),
    })
}
        ...
        event: Some(server_event::Event::Message(MessageEvent {
            ...
            message: Some(ProtoRoutedMessage {
                metadata: Some(ProtoRoutingMetadata {
                    ...
                    recipient_type: metadata.recipient_type.as_deref().map(str::to_owned),
                    recipient_identity: metadata.recipient_identity.as_deref().map(str::to_owned),
