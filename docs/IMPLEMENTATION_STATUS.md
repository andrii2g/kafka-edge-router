...
| Tenant-isolated subscriptions | implemented | core rejects tenant mismatches; WS, SSE, gRPC, and publish adapters authorize then rewrite tenant input to the authenticated principal; protocol tests cover cross-tenant subscription and publish denial |
...
| Health/readiness/status/metrics | implemented | HTTP/gRPC health, bounded Prometheus counters/gauges/histograms, Kafka lag and assignment, protocol cardinalities, and optional hysteretic Kafka readiness are wired and tested |
...
| Production deployment files | implemented | production Kustomize base and RC overlay include TLS proxying, unique pod group ids, external Secrets, probes, disruption, resources, topology, and egress policy; local rendering and K3s server-side dry-run pass, while the candidate rollout remains a release gate |
...
| Load, soak, SBOM, signed release | scaffolded | bounded multi-protocol load and soak tooling, profiling scripts, signed multi-architecture workflow, SBOM/provenance/checksums, rollback runbook, and RC notes exist; multi-hour K3s evidence, tag publication, and game day remain release gates |
...
| 007: Durable webhooks | implemented | ADR 0005, explicit mode split, pre-commit durable fan-out, persisted retry/DLQ state, ownership groups, audit-safe metrics, runbook, and required real-Kafka restart/crash recovery tests in the broker-backed CI command satisfy the acceptance criteria |
| 008: Production observability | implemented | fixed-label metrics and seven latency stages, Kafka lag/assignment/commit/rebalance telemetry, bounded W3C trace propagation, nonfatal OTLP export, graceful flush, hysteretic readiness, dashboard, alerts, runbook, and Criterion overhead measurements satisfy the acceptance criteria |
| 009: Security hardening | implemented | reloadable asymmetric JWT/JWKS, scoped authorization, loopback-only protected proxy mode, proxy mTLS mapping, DNS-pinned webhook egress, abuse caps, fuzzing, and supply-chain scanning satisfy the acceptance criteria |
| 010: Benchmark, soak, package, and release | scaffolded | expanded Criterion and bounded end-to-end tooling, measured local baseline, production Kustomize overlay, non-root image build, release workflow, release notes, and rollback/game-day runbooks are implemented; multi-hour K3s soak, profiles, signed tag artifacts, rollback exercise, and game day require the candidate image and cluster Secrets |
