...
| 007: Durable webhooks | implemented | ADR 0005, explicit mode split, pre-commit durable fan-out, persisted retry/DLQ state, ownership groups, audit-safe metrics, runbook, and real-Kafka restart recovery satisfy the acceptance criteria |
| 008: Production observability | implemented in current working tree | fixed-label metrics and seven latency stages, Kafka lag/assignment/commit/rebalance telemetry, bounded W3C trace propagation, nonfatal OTLP export, graceful flush, hysteretic readiness, dashboard, alerts, runbook, and Criterion overhead measurements satisfy the acceptance criteria |
